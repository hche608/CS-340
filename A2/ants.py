...
class Ants():
    ...
    def __init__(self, local_root, local_files, remote_root,
                 SYNC, TIME_FORMAT, verbose=False):
        self.SYNC = SYNC
        ...
        common_files = list(filter(lambda f: not f.startswith('.') and not f.endswith(
            '~') and not f == self.SYNC and os.path.isfile(os.path.join(local_root, f)), list(d1 & d2)))
        # Determine the items that only exist in left directory
        only = list(filter(lambda f: not f.startswith('.') and not f.endswith(
            '~') and not f == self.SYNC and os.path.isfile(os.path.join(local_root, f)), list(d1 - d2)))

        # Compare the directories
        match, mismatch, errors = filecmp.cmpfiles(
            local_root, remote_root, common_files, shallow=False)
        if self.verbose:
            if match:
    ...
    def needs_copy(self, fname, local_info, remote_info):
        ...
        if remote_info and fname in remote_info:
            ...
            local_mtime = time.strptime(
                local_info[fname][0][0], self.TIME_FORMAT)
            local_uid = local_info[fname][0][1]
            remote_mtime = time.strptime(
                remote_info[fname][0][0], self.TIME_FORMAT)
            ...
            if remote_uid == 'deleted' and len(local_info[fname]) > 1 and local_info[
                    fname][1][1] == 'deleted':
                result = True
                ...
                if remote_uid in [j for i in local_info[fname] for j in i]:
                    ...
                    result = True
            # Case 1: same UID, DIff Mtime => Earliest time apply
            elif local_mtime < remote_mtime and local_uid == remote_uid:
                result = True
            # Case 2: diff UID, DIff Mtime => latest time apply
            elif local_mtime > remote_mtime and local_uid != remote_uid:
