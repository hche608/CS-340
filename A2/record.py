...
class Record():
    ...
    def insert(self, root, fname):
        self.records.insert(0, [time.mtime(os.path.getmtime(
            os.path.join(root, fname))), self.sha256(os.path.join(root, fname))])

    def exists(self, searching_key):
        return searching_key in [
            elem for sublist in self.records for elem in sublist]
